...

{
    ...
    {
        public ListViewModel(IPostRepository postRepository, int p)
        {
            Posts = postRepository.Posts(p - 1, 10);
            TotalPosts = postRepository.TotalPosts();
        }

        public ListViewModel(IPostRepository postRepository, ITagRepository tagRepository,
                             ICategoryRepository categoryRepository, string text, string type, int p)
        {
            switch (type)
            {
                case "Category":
